...
`req` 命令会自动读取已经保存的 `baseUrl` 和鉴权 `headers` 整合到请求上，然后调用 `axios.request()` 发送请求。

- 第一个参数：HTTP method
- 第二个参数：请求 pathname，以 `/` 开头，可以包含查询参数
- 第三个参数：可选 JSON 请求体
...
```bash
req GET '/users'
req GET '/users?page=1&limit=20'
req POST '/action' '{"id":"123"}'
```
